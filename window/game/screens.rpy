...
# 主游戏界面
screen main_interface():  #定义名为`main_interface`的屏幕。
    add "bg city"    # 添加背景

    # 右下角设置图标
    imagebutton:   #定义一个图片按钮。
    #按钮正常状态时显示`icon settings`图标，并使用单位矩阵（即原图）的颜色。
        idle Transform("icon settings", matrixcolor=IdentityMatrix())
        #鼠标悬停时，使用增加亮度0.2的变换（变亮）。
        hover Transform("icon settings", matrixcolor=BrightnessMatrix(0.2))
    #点击按钮时，显示悬浮窗口屏幕`floating_window`和背景虚化层`blur_overlay`。 
        action [Show("floating_window"), Show("blur_overlay")]
        #将按钮放置在屏幕右下角(x轴方向0.95，y轴方向0.7)
        align (0.97, 0.70)
        at button_hover     #应用之前定义的按钮悬停动画（缩放效果）。
        # 悬停提示
        hovered Show("tooltip", text="打开设置")  #鼠标悬停时显示提示工具（tooltip）屏幕，并传递文本"打开设置"。
        unhovered Hide("tooltip")     #鼠标离开时隐藏提示工具。

# 背景虚化层
screen blur_overlay():
    zorder 98   #设置该屏幕的显示层级为98（较低，位于主界面和悬浮窗口之间）。
    add "bg city":
        at blur_transform      #应用之前定义的虚化变换。
        alpha 0.7           #设置透明度为70%，使背景变暗。
    # 添加半透明黑色层增强虚化效果，添加一个半透明的深灰色矩形（#00000080表示黑色，80是十六进制透明度，50%不透明），叠加在背景上增强虚化效果。
    add Solid("#00000080")

# 悬浮窗口
screen floating_window():
    zorder 100   #设置显示层级为100（较高，位于其他屏幕之上）。
    modal True  # 设置为模态，阻止与背景屏幕的交互。
    # 窗口容器
    frame:
        style "floating_frame" #应用样式`floating_frame`（在gui.rpy中定义）。
        xalign 0.5
        yalign 0.5      #居中显示
        xsize 1200
        ysize 800         #设置窗口大小。
        padding (30, 40)    #内边距
        at float_in       #应用悬浮窗口出现的动画。
        # 标题栏
        hbox:          #水平盒子，用于放置标题和关闭按钮。
            xfill True
            spacing 10
            
            # 标题
            text "系统设置":      #标题文本。
                style "floating_title"
                   
            # 关闭按钮（右对齐）
            null width 0.8     #占位空间，将关闭按钮推到右侧。
            imagebutton:     # 关闭按钮，与设置按钮类似。
                idle Transform("icon close", matrixcolor=IdentityMatrix())
                hover Transform("icon close",matrixcolor=BrightnessMatrix(0.2))
                action [Hide("floating_window"), Hide("blur_overlay")]
                align (1.0, 0.0)
                at button_hover
        # 分隔线
        add Solid("#00000080"):   #添加一条分隔线。
            xsize 700
            ysize 2
            yoffset 40
# 提示信息
screen tooltip(text):   #定义提示工具屏幕，接受一个文本参数。
    zorder 101       #设置显示层级最高（在最上层）。
    frame:              #创建一个框架作为提示框。
        background "#000c"
        padding (10, 5)
        xalign 0.95
        yalign 0.95
        yoffset -50  # 位置在右下角（xalign 0.95, yalign 0.95），并向上偏移50像素（yoffset -50）。
        at transform:
            alpha 0.0
            easein 0.2 alpha 1.0  #使用淡入动画（从透明到不透明）。
        text text:
            size 18
            color "#ffffff"    # 内部包含一个文本，显示传入的提示内容。
