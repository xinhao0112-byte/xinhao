...
# 在 gui.rpy 的适当位置添加以下样式

style floating_frame:               #定义悬浮窗口框架的样式。
    background Frame("images/gui.jpg", 25, 25)  # 使用图片背景，并使用Frame函数使其具有圆角边框（25像素的边框半径）。
    padding (25, 25)  #内边距25像素
    xmargin 0
    ymargin 0         #外边距为0。

style floating_title:     #定义标题文本样式。
    size 32                #字体大小32
    color "#ffffff" 
    outlines [(2, "#0008", 0, 0)]   #黑色描边（宽度2，颜色#0008（半透明黑））。
    bold True        #粗体。

style floating_button:      #定义悬浮窗口中按钮的样式。
    background Frame("#4a7", 5, 5)   #正常状态背景为绿色（#4a7），圆角5。
    hover_background Frame("#6c9", 5, 5)  #悬停状态背景为浅绿色（#6c9）。
    padding (20, 8)   #内边距。
    min_width 20       #最小宽度120像素。
    size 12
    color "#fff"
